...
# script ends - so a successful deploy and a `die` in the safety block below look exactly
# alike from outside: a window that flashes and is gone. Hold the window open in that case
# only. git-bash.exe starts an interactive shell ($- contains i); `./deploy.sh` from a
# terminal or from another script does not, so nothing here ever blocks a non-interactive run.
if [[ $- == *i* ]]; then
